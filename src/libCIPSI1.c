...
IMAGE erosionImageSE(IMAGE img, ELEMENT_STRUCTURANT SE)
{
	...
	for (int i = 0; i < img.Nblig; i++)
	{
		for (int j = 0; j < img.Nbcol; j++)
		{
			...
			for (int k = -SE.rayon; k <= SE.rayon; k++)
			{
				for (int l = -SE.rayon; l <= SE.rayon; l++)
				{
					int x = i + k;
					int y = j + l;
					...
					if ((x >= 0 && x < img.Nblig) && (y >= 0 && y < img.Nbcol))
					{
						...
						if (SE.pixel[k + SE.rayon][l + SE.rayon] != 0)
						{
							produit = SE.pixel[k + SE.rayon][l + SE.rayon] * img.pixel[x][y];
...
IMAGE ouvertureImageSE(IMAGE img, ELEMENT_STRUCTURANT SE)
{
	...
	imgOpen = dilatationImageSE(imgErode, SE);
...
{
	IMAGE imgClose = allocationImage(img.Nblig, img.Nbcol);
	IMAGE imgDilate = dilatationImageSE(img, SE);
	imgClose = erosionImageSE(img, SE);
...
IMAGE blackTopHatSE(IMAGE img, ELEMENT_STRUCTURANT SE)
{
	IMAGE BlackTopHat = allocationImage(img.Nblig, img.Nbcol);
	IMAGE imgTemp = allocationImage(img.Nblig, img.Nbcol);
	imgTemp = dilatationImageSE(img, SE);
...
{
	//Initialise l'élement structurant
	...
	if (SE.type == 'disk') //Création d'un ES de type disque
	{
		taille_SE = SE.hauteur * SE.largeur;
		SE.data = (unsigned char*)calloc(taille_SE, sizeof(unsigned char));
		if (SE.data == NULL)
		{
			return(SE);
		}
		SE.pixel = (unsigned char**)malloc(SE.largeur * sizeof(unsigned char*));
		if (SE.pixel == NULL) {
			free(SE.data);
			SE.data = NULL;
			return(SE);
		...
		for (i = 0; i < SE.largeur; i++)
		{
			SE.pixel[i] = &SE.data[i * ((2 * SE.rayon) + 1)];
		}
		...
		remplissage4(SE.pixel, SE.rayon, SE.rayon, 0, 1); //Permet de remplir le disque
	}

	//Retourne notre element structurant
	return(SE);
}
...
float Traitement(const char* nomImg, int inversion)
{
	...
	if (inversion == 1) //Traitement morphologique des images In
	{
		...
		ELEMENT_STRUCTURANT SE = allocation_SE('disk', 20);

		imgTraitement = whiteTopHatSE(imgTraitement, SE);
		imgTraitement = seuillageOtsu(imgTraitement);

		SE = allocation_SE('disk', 2);
		imgTraitement = erosionImageSE(imgTraitement, SE);
		SE = allocation_SE('disk', 3);
		imgTraitement = dilatationImageSE(imgTraitement, SE);
