...
IMAGERGB bruitAleatoireImageRGB(IMAGERGB img, int amplitude)
{
	...
	for (int i = 0; i < img.Nbcol * img.Nblig; i++)
	{
		...
		if (bruit.data[i].R > 255) bruit.data[i].R = 255;
		if (bruit.data[i].G > 255) bruit.data[i].G = 255;
		if (bruit.data[i].B > 255) bruit.data[i].B = 255;
		if (bruit.data[i].R < 0) bruit.data[i].R = 0;
		if (bruit.data[i].G < 0) bruit.data[i].G = 0;
		if (bruit.data[i].B < 0) bruit.data[i].B = 0;
	}
	return bruit;
}
